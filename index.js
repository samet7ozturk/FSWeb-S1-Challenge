...
console.log(fenomenler[3].profile);
...
function indekseGoreFenomen(fenomen, ind) {
  return `${ind}. indekste bulunan fenomen: ${fenomenler[ind].profile}`;
}
...
function fenomenSil(fenomen, ind) {
  const yeniDizi = [];
  for (let i = 0; i < fenomen.length; i++) {
    yeniDizi.push(fenomen[i]);
  }
...
function enFenomenler(fenomen) {
  const yeniDizi = [];
  for (const paket of fenomen) {
    const followers = paket.followers;
    if (followers >= 100000000) {
      yeniDizi.push(paket.profile);
    }
  }
  return yeniDizi;
}
...
function fenomenGonderimSayisi(fenomen, profil) {
  let gönderimSayisi = 0;
  for (const paket of fenomen) {
    const fenomenProfil = paket.profile;
    if (fenomenProfil == profil) {
      gönderimSayisi = paket.posts;
    }
  }
  return gönderimSayisi;
}
